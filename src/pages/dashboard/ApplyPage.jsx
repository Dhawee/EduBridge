...
function ApplyPage() {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      const email = formData.email || "student@edubridge.com";
      const reference = `APP-${Date.now()}`;
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto text-white">
      <h2 className="text-3xl font-bold mb-4">Apply to {selectedSchool.name}</h2>

      <form
        onSubmit={handleSubmit}
        className="bg-gray-900 p-6 rounded-xl border border-gray-700 grid gap-4"
      >
        <input
          name="fullName"
        ...
        <button
          type="submit"
          className="bg-green-600 py-2 rounded font-semibold hover:bg-green-700"
        >
          Submit & Pay ₦{APPLICATION_FEE.toLocaleString()}
        </button>
      </form>
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
            >
              <h3 className="text-xl mb-4">Confirm Payment</h3>

              {paymentError && (
                <p className="text-red-400 mb-2">{paymentError}</p>
              )}
      ...
      <AnimatePresence>
        {showSuccessModal && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
          >
            <motion.div className="bg-gray-900 p-8 rounded-xl text-center">
