...
function ApplyPage() {
  ...
  // Load draft and profile data
  useEffect(() => {
    const draft = localStorage.getItem("applyDraft");
    const userData = JSON.parse(localStorage.getItem("user") || "{}");
    
    // Prepare form data from draft or profile
    const dataToLoad = draft ? JSON.parse(draft) : {
      ...initialFormData,
  ...
  if (!selectedSchool) {
    return <p className="p-6 text-red-500">No school selected. Please go back and select one.</p>;
  }
  ...
  const confirmSubmit = async () => {
    ...
    try {
      const userEmail = formData.email || localStorage.getItem("userEmail") || "student@edubridge.com";
      ...
      // If payment successful, save application
      if (paystackResponse && paystackResponse.status) {
        ...
        setShowModal(false);
        setSuccessMessage("Payment successful! Your application has been submitted.");
    ...
    <div className="p-6 max-w-3xl mx-auto text-white">
      <div className="mb-6">
        ...
        <div className="bg-blue-600/20 border border-blue-500 p-4 rounded-lg">
          <p className="text-lg font-semibold text-blue-300">School: <span className="text-white">{selectedSchool.name}</span></p>
          <p className="text-sm text-blue-200 mt-2">Application Fee: ₦{APPLICATION_FEE.toLocaleString()}</p>
        </div>
      </div>
      ...
      >
        ...
        {/* Profile Information - Auto-filled Display */}
        <div className="bg-green-600/10 p-4 rounded-lg border border-green-500">
          <p className="text-green-300 text-sm mb-3 font-semibold">Your Profile Information</p>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <p className="text-gray-400 text-sm">Full Name</p>
              <p className="text-white text-lg font-semibold">{formData.fullName || "Not found"}</p>
            </div>
            <div>
              <p className="text-gray-400 text-sm">Email</p>
              <p className="text-white text-lg font-semibold">{formData.email || "Not found"}</p>
            </div>
          </div>
        </div>

        <select name="year" value={formData.year} onChange={handleChange} className="w-full border p-2 rounded bg-gray-800">
        ...
        <select name="term" value={formData.term} onChange={handleChange} className="w-full border p-2 rounded bg-gray-800">
        ...
        <div className="flex justify-between mt-6 items-center">
          <button type="button" onClick={saveDraft} className="px-4 py-2 bg-yellow-500 rounded text-black font-semibold hover:bg-yellow-600 transition">Save Draft</button>
          <button type="submit" className="px-4 py-2 bg-green-500 rounded text-black font-semibold hover:bg-green-600 transition">Submit & Pay</button>
        </div>
      </form>
      ...
      <AnimatePresence>
        {showModal && (
          <motion.div className="fixed inset-0 bg-black/60 flex items-center justify-center px-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="bg-gray-900 p-8 rounded-xl max-w-md w-full border border-gray-700 shadow-xl" initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }}>
              <h3 className="text-2xl font-bold mb-4">Confirm & Pay</h3>
              <div className="bg-gray-800 p-4 rounded-lg mb-4">
                <p className="text-gray-300 mb-2">Application Fee:</p>
                <p className="text-3xl font-bold text-green-400">₦{APPLICATION_FEE.toLocaleString()}</p>
              </div>
              <p className="text-gray-300 mb-6">You will be redirected to Paystack to complete payment securely.</p>
        ...
        {showSuccessModal && (
          ...
          >
            ...
            >
              ...
              <p className="text-gray-400 text-sm">Redirecting to your applications...</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
